...
def _project_root() -> Path:
    """
    /home/anil/dbvulscan/report_service/report_domain_detail.py
...
def _pdf_path(report_dir: Path, domain_run_id: int) -> Path:
    ...
    return report_dir / f"domain_run_{domain_run_id}_detail.pdf"


def _tmp_header_pdf_path(report_dir: Path, domain_run_id: int) -> Path:
    return report_dir / f".tmp_domain_run_{domain_run_id}_detail_header.pdf"
...
def _css_inline() -> CSS:
    # Domain Summary ile aynı heading/cosmetic + header standard font fix
    return CSS(
        string=r"""
@page {
  size: A4;
...
<head>
  <meta charset="utf-8" />
  <title>Domain Run Detail Report</title>
</head>
<body>

  <div class="title">Domain Run Detail Report</div>
  ...
  <div class="section">
    ...
    <div class="small-note">
      Generated by report_service.report_domain_detail
    </div>
  </div>
...
def generate(domain_run_id: int, output_dir: Optional[str] = None, force: bool = False) -> str:
    """
    Domain Run Detail PDF üretir ve aynı PDF'in devamına
    her bir assessment_run için report_assessment_detail.generate(run_id) çıktısını ekler (merge).

    - report_dir: settings(setting_key='report_dir') veya output_dir override
    - cache: domain_run_{domain_run_id}_detail.pdf (COMBINED)
    ...
    try:
        ...
        # Header PDF (Domain Info + Assessment Results)
        ...
        # Each assessment -> detail PDF
        try:
            from report_service import report_assessment_detail
        except Exception as e:
            raise RuntimeError("report_service.report_assessment_detail import edilemedi.") from e

        assessment_pdfs: List[Path] = []
        for run_id in run_ids:
            p = report_assessment_detail.generate(run_id, output_dir=str(rep_dir), force=force)
            assessment_pdfs.append(Path(p))

        # Merge: header + all assessment detail pdfs
